...
public class AndroidDevice implements Comparable<AndroidDevice> {
	...
	public AndroidDevice(IDevice target) {
		this.target = target;
		log = LoggerFactory.getLogger(AndroidDevice.class.getName() + ":" + target.getSerialNumber());
	}
	...
	public void scroll(Point p, Point p2) {
		try {
			String cmd = String.format(Locale.ENGLISH, "input swipe %.3f %.3f %.3f %.3f", p.getX(), p.getY(), p2.getX(), p2.getY());
			log.debug("SEND: "+cmd);
			target.executeShellCommand(cmd, new IShellOutputReceiver() {
                @Override
                public void addOutput(byte[] data, int offset, int length) {
