...
import com.ribomation.droidAtScreen.cmd.TemplateMatchingCommand;
...
public class DeviceFrame extends JFrame implements Comparable<DeviceFrame> {
	...
	public DeviceFrame(Application app, final AndroidDevice device) {
		this.app = app;
		this.device = device;
		this.log = LoggerFactory.getLogger(DeviceFrame.class.getName() + ":" + device.getName());
	...
	class Retriever extends TimerTask {
		...
		public void run() {
			...
			if (fresh) {
				log = LoggerFactory.getLogger(DeviceFrame.class.getName() + ":" + device.getName());
				setTitle(device.getName());
				pack();
				applySkin();
	...
	protected JComponent createToolBar() {
		...
		buttons.add(new TemplateMatchingCommand(this).newButton());
	...
	public void setTemplateMaching(boolean templateMatching) {
		this.templateMatching = templateMatching;
	...
	public void scroll(Point p1, Point p2) {
		// TODO Auto-generated method stub
		device.scroll(p1, p2);
	}
